<?php
/*
 $Id: ib_handler.php VER: 1.0.3414 $
...
$customer_id = null;
$my_order_query = null;
$txid = isset ( $HTTP_POST_VARS['txid'] ) ? $HTTP_POST_VARS['txid'] : 'n/a'  ;
$invoice_reference = isset ( $HTTP_POST_VARS['txid'] ) ? $HTTP_POST_VARS['txid'] : 'n/a'  ;
$iostabc = 'https://www.iostabc.com/tx/'.$txid ;
//*************************************
// Validate request
//
...
if ($ok)
{
    if (! isset ($HTTP_POST_VARS["checksum"]) || ! isset ($HTTP_POST_VARS["invoice_created_at"]) || ! isset ($HTTP_POST_VARS["invoice_status"]))
    {
        $ok = false;
...
if ($ok)
{
    ...
    "invoice_reference"=>$HTTP_POST_VARS['txid'],
...
if ($ok)
{
    ...
    {
        $ok = false;
        $result = 'IOSTWALLET transaction value ('.tep_output_string_protected($HTTP_POST_VARS['invoice_amount']).') does not match order value ('.number_format($total['value']*$order['currency_value'], $currencies->get_decimal_places($order['currency'])).')';
    }
}
if ($ok)
{
    ...
    $order = $my_order;
    $delivered_status = 3;
    if (($order['orders_status'] == MODULE_PAYMENT_IOSTWALLET_COMP_ORDER_STATUS_ID) || ($order['orders_status'] == $delivered_status))
    {
        $ok = false;
...
if ($ok) {
    ...
    if ( ($HTTP_POST_VARS["invoice_status"] == "pending") || ($HTTP_POST_VARS["invoice_status"] == "created") ) {
        $ok = true;
    } else if ( $HTTP_POST_VARS["invoice_status"] == "approved" ) {
        $ok = true;
    } else if ( $HTTP_POST_VARS["invoice_status"] == "sum_too_low" ) {
        $ok = true;
    }
	if (!$ok)
	{
 ...
 if( isset( $HTTP_POST_VARS['custom'] ))	{
   
        $customer_id  = $HTTP_POST_VARS['custom'] ;
     
     $whos_online_query = tep_db_query("select time_last_click, session_id from whos_online where customer_id = '" . $customer_id . "'");


    $order_time_query = tep_db_query("select date_purchased, customers_name, customers_email_address from orders where orders_id = '" . $HTTP_POST_VARS['order_id'] . "'");
...
if ($result == 'VERIFIED')
{
    ...
    switch($HTTP_POST_VARS["invoice_status"])
    {
        case "created":
        case "pending":
            $msg = "customer has been asked to pay ".$HTTP_POST_VARS['invoice_amount']." ".$HTTP_POST_VARS['invoice_currency'] .
            $order_status_id = MODULE_PAYMENT_IOSTWALLET_CREATE_ORDER_STATUS_ID;
            break;
        case "approved":
            $msg = "IOSTWALLET has confimed that the payment of ".$HTTP_POST_VARS['invoice_amount']." ".$HTTP_POST_VARS['invoice_currency'] .
             $refTx =    '
                        Reference TX:  '.$iostabc.'  ' ; 
            $order_status_id = MODULE_PAYMENT_IOSTWALLET_COMP_ORDER_STATUS_ID ;
            $invoice_approved = true;
            break;
        case "sum_too_low":

            $msg = 'Invoice insufficient balance by iostwallet' ;
            $refTx =    '
                        Reference TX:  '.$iostabc.'  ' ; 
            
            $order_status_id = MODULE_PAYMENT_IOSTWALLET_SUM_TOO_LOW_ORDER_STATUS_ID;
            break;
    }
    $comment_status .= $msg ;
    $customer_notified = '0';
    ...
    'comments'=> ucfirst($HTTP_POST_VARS['invoice_status']).'['.$comment_status.']'. $refTx);
            ...
            if ((MODULE_PAYMENT_INPAY_DECREASE_STOCK_ON_CREATION=='False') && (STOCK_LIMITED == 'true'))
            {
                if (DOWNLOAD_ENABLED == 'true')
                {
                    $stock_query_raw = "SELECT products_quantity, pad.products_attributes_filename 
                                    FROM 'productsl' p
                                    LEFT JOIN ".'orders_products_attributes'." pa
                                    ON p.products_id=pa.products_id
                                    LEFT JOIN ".'orders_products_download'." pad
                ...
                {
                    $stock_query = tep_db_query("select products_quantity from 'productsl' where products_id = '".tep_get_prid($order->products[$i]['id'])."'");
                }
                if (tep_db_num_rows($stock_query) > 0)
                {
                    ...
                    tep_db_query("update 'productsl' set products_quantity = '".$stock_left."' where products_id = '".tep_get_prid($order->products[$i]['id'])."'");
                    if (($stock_left < 1) && (STOCK_ALLOW_CHECKOUT == 'false'))
                    {
                        tep_db_query("update 'productsl' set products_status = '0' where products_id = '".tep_get_prid($order->products[$i]['id'])."'");
                    }
                }
            } // decrease stock end

            // Update products_ordered (for bestsellers list)
            tep_db_query("update 'productsl' set products_ordered = products_ordered + ".sprintf('%d', $order->products[$i]['qty'])." where products_id = '".tep_get_prid($order->products[$i]['id'])."'");
...
} else
{
    ...
    // send warning email
    //
    if (tep_not_null(MODULE_PAYMENT_IOSTWALLET_DEBUG_EMAIL))
    {
        ...
        tep_mail('', MODULE_PAYMENT_IOSTWALLET_DEBUG_EMAIL, 'IOSTWALLET Invalid Process', $email_body, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
    }
    //
    ...
    if ( isset ($HTTP_POST_VARS['order_id']) && is_numeric($HTTP_POST_VARS['order_id']) && ($HTTP_POST_VARS['order_id'] > 0))
    {
        ...
        if (tep_db_num_rows($check_query) > 0)
        {
            ...
            'date_added'=>'now()',
            'customer_notified'=>'0',
            'comments'=> ' Invalid ['.$comment_status.']');
            tep_db_perform('orders_status_history', $sql_data_array);
        }
    }
}
